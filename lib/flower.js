(function (root, factory) {
	...
	Flower.prototype.on = function (subscribeToLatestEvents, eventName, handler, subscriber, invokeImmediatelyIfPossible) {
		...
		for (var i = 0; i < eventNames.length; i += 1) {
			...
			if (invokeImmediatelyIfPossible && (eventName in this._wasTriggeredWith)) {
				this._executeHandler(handler, subscriber, this._wasTriggeredWith[eventName])
			}
			// function body Logic
	...
	Flower.prototype.onWithPast = function (eventName, handler, context) {
